...
import { api, events, types } from './proto/ldk-server';
...
export class LdkServerService implements LightningProvider {
  ...
  async verifyConnection(connection: LdkServerClient): Promise<void> {
    await this.getWalletInfo(connection);
  }

  getSubscriptionConnection(_connection: LdkServerClient) {
    return null;
  }

  subscribeEvents(connection: LdkServerClient): EventEmitter {
    return connection.subscribe(
      Endpoints.SUBSCRIBE_EVENTS,
  ...
  async getRouteToDestination(
    _connection: LdkServerClient,
    _options: GetRouteToDestinationOptions
  ...
  async getPayments(connection: LdkServerClient, options: GetPaymentsOptions) {
    const resp = await listPayments(connection, options);
  ...
  async getAccessIds(_connection: LdkServerClient) {
    throw new Error(NOT_SUPPORTED);
  }

  async bakeMacaroon(
    _connection: LdkServerClient,
    _options: BakeMacaroonOptions
...
async function listPayments(
  connection: LdkServerClient,
  options: GetPaymentsOptions | GetInvoicesOptions
): Promise<api.IListPaymentsResponse> {
  try {
    ...
    if (message.includes('Failed to decode payment')) {
      return { payments: [] };
    }
    throw error;
  }
